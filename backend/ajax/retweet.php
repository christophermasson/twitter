...
if(is_post_request()){
    ...
    if(isset($_POST['retweetId']) && !empty($_POST['retweetId'])){
        $retweetBy=h($_POST['retweetBy']);
        $tweetID=h($_POST['retweetId']);
        $retweetText="Retweet";
        $retweetClass="retweet-it";
        ...
        if(!empty($retweetData)){
            if($retweetData->retweetBy===$retweetBy && $retweetData->retweetFrom===$tweetID){
                ...
                $retweetClass="retweeted-it";
            }
        }

        echo ' <div class="retweet-wrapper">
                    <div class="retweet-wrapper-content">
                        <div role="menuitem" data-focusable="true" tabindex="0" class="menuItem '.$retweetClass.'">
